...
10. **Google OAuth uses redirect flow**: Google sign-in MUST use `supabase.auth.signInWithOAuth()` redirect — NOT GIS popup mode. GIS popups are blocked by Google's own `Cross-Origin-Opener-Policy: same-origin` header which breaks `postMessage`. Never use `signInWithIdToken` or `google.accounts.id` APIs.
