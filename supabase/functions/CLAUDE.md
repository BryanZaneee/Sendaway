...
| process-notifications | Sends queued scheduled-confirmation emails via Resend with retry/backoff | Triggered every 5 minutes by cron-job.org |
...
| _shared/cron-auth.ts | Verifies CRON_SECRET header matches environment variable | Guards process-notifications, process-delivery, and cleanup-logs from unauthorized invocation |
